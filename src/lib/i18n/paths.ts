...
export function translatePath(path: string, locale: Locale): string {
  ...
  if (blogSlug) {
    // Check if we have a translation mapping for this blog post
    const translatedSlug = blogSlugTranslations[blogSlug]?.[locale];
    if (translatedSlug) {
      // Use the translated slug
      const prefix = getLocalePrefix(locale);
      return `${prefix}/blog/${translatedSlug}`;
    }
    // No translation available - redirect to blog index for that locale
    const prefix = getLocalePrefix(locale);
    return `${prefix}/blog`;
  }

  const key = (Object.entries(pathKeyByLocalePath) as [Locale, Record<string, string>][]).reduce<
    string | undefined
  >((found, [, mapping]) => {
